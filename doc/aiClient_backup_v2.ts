  ...
  static async sendCommand(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 429 || errorBody.includes('overloaded')) {
          return {
            content: '',
            error: 'The AI service is currently overloaded. Please try again in a few moments with a shorter trip description.'
          };
        }
