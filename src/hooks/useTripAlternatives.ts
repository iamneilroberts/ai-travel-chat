...
export const useTripAlternatives = (content: string, systemPrompt: string) => {
  ...
  const parseOptions = useCallback((blocks: string[]) => {
    ...
    return validOptions;
  }, [setCommandError, setLastResponse, setTripAlternatives]);

  const parseNewTripOptions = useCallback((responseContent: string, rawRequest?: string, rawResponse?: unknown) => {
  ...
  }, [parseOptions, setCommandError, setLastResponse]);
  ...
  const processCommand = useCallback(async (
    ...
    try {
      commandResult = CommandProcessor.processCommand(
        ...
        undefined, // savePath
        rejectionNote,
        undefined, // detailedPlanRejectionNote
        alternatives,
        optionNumber
      );
  ...
  }, [content, systemPrompt, parseNewTripOptions]);
